...
class Trainer(object):
    ...
    def __init__(self, cfg, model, train_data_iter, test_data_iter, optimizer, save_dir, device):
        ...
        self.test_data_iter = test_data_iter # iterator to load data
        self.optimizer = optimizer
        self.save_dir = save_dir
        self.device = device # device name

    def train(self, get_loss, model_file=None, pretrain_file=None, n_epochs = 1, data_parallel=False):
        """ Train Loop """
    ...
    def eval(self, evaluate, model_file=None, data_parallel=False):
        """ Evaluation Loop """
    ...
    def load(self, model_file, pretrain_file):
        ...
        if model_file:
            ...
            self.model.load_state_dict(torch.load(model_file, self.device), strict = False)
            ...
            if pretrain_file.endswith('.ckpt'): # checkpoint file in tensorflow
                ...
                self.model.transformer.load_state_dict(
                    {key[12:]: value
                        for key, value in torch.load(pretrain_file, self.device).items()
